...
<div id="tb" style="padding:5px;height:auto">
	<form id="searchFrom" action="">
		...
		<input type="text" name="filter_EQL_customerName" class="easyui-combobox" data-options="
			prompt: '客户名称',
...
$(function(){
	dg=$('#dg').datagrid({
		method: "get",
		url:'${ctx}/domesticTrade/agentPurchase/json',
		fit : true,
		fitColumns : true,
		border : false,
		striped:true,
		...
		columns:[[
			...
			{field:'contractNo',title:'合同编号',sortable:true,width:10},
			{field:'customerName',title:'客户名称',sortable:true,width:10,
			...
			{field:'agencyFee',title:'代理费',sortable:true,width:10},
			{field:'guartee',title:'担保',sortable:true,width:10},
			{field:'adress',title:'合同管理地',sortable:true,width:10},
			{field:'remark',title:'备注',sortable:true,width:10},
		    {field:'logScheme',title:'物流方案',sortable:true,width:15},
...
function add() {
	d=$("#dlg").dialog({
		fit:true,
	    title: '添加代理采购合同',
	    href:'${ctx}/domesticTrade/agentPurchase/create',
	    modal:true,
	    closable:false,
	    style:{borderWidth:0},
	    buttons:[{
...
function upd() {
	...
	d=$("#dlg").dialog({
		fit:true,
	    title: '修改代理采购合同',
	    href:'${ctx}/domesticTrade/agentPurchase/update/' + row.id,
	    modal:true,
	    closable:false,
	    style:{borderWidth:0},
	    buttons:[{
...
function del(){
	...
	parent.$.messager.confirm('提示', '删除后无法恢复您确定要删除？', function(data){
		if (data){
			$.ajax({
				type:'get',
				url:"${ctx}/domesticTrade/agentPurchase/delete/" + row.id,
				success: function(data){
					successTip(data,dg);
				}
			});
...
function detail(){
	...
	d=$("#dlg").dialog({
		fit:true,
	    title: '查看明细',
	    href:'${ctx}/domesticTrade/agentPurchase/detail/' + row.id,
	    modal:true,
	    closable:false,
	    style:{borderWidth:0},
	    buttons:[{
...
function apply(id){
	...
	applyDlg=$("#applyDlg").dialog({
		...
		width: 680,
	    height: 300,
	    href:"${ctx}/domesticTrade/agentPurchase/apply/" + id,
	    maximizable:false,
	    modal:true
	});
}
...
function callBack(){
	...
	parent.$.messager.confirm('提示', '您确定要撤回申请？', function(data){
		if (data){
			...
			$.ajax({
				type:'get',
				url:"${ctx}/domesticTrade/agentPurchase/callBack/" + row.id + "/" + row.processInstanceId,
				success: function(data){
			    	successTip(data,dg);
					if(data=='success'){
