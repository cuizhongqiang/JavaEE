...
<div id="tb" style="padding:5px;height:auto">
	...
	<shiro:hasPermission name="sys:domesticPurchase:add">
		<a href="#" class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="add()">添加</a>
		<span class="toolbar-item dialog-tool-separator"></span>
	</shiro:hasPermission>
	<shiro:hasPermission name="sys:domesticPurchase:delete">
	    <a href="#" class="easyui-linkbutton" iconCls="icon-remove" plain="true" onclick="del()">删除</a>
	    <span class="toolbar-item dialog-tool-separator"></span>
	</shiro:hasPermission>
	<shiro:hasPermission name="sys:domesticPurchase:update">
	    <a href="#" class="easyui-linkbutton" iconCls="icon-edit" plain="true" onclick="upd()">修改</a>
	    <span class="toolbar-item dialog-tool-separator"></span>
	</shiro:hasPermission>
	<shiro:hasPermission name="sys:domesticPurchase:detail">
		<a href="#" class="easyui-linkbutton" iconCls="icon-search" plain="true" onclick="detail()">查看明细</a>
		<span class="toolbar-item dialog-tool-separator"></span>
	</shiro:hasPermission>
	<shiro:hasPermission name="sys:domesticPurchase:apply">
		<a href="#" class="easyui-linkbutton" iconCls="icon-redo" plain="true" onclick="apply()">提交申请</a>
		<span class="toolbar-item dialog-tool-separator"></span>
	</shiro:hasPermission>
	<shiro:hasPermission name="sys:domesticPurchase:callBack">
		<a href="#" class="easyui-linkbutton" iconCls="icon-back" plain="true" onclick="callBack();">撤回申请</a>
		<span class="toolbar-item dialog-tool-separator"></span>
	</shiro:hasPermission>
	<shiro:hasPermission name="sys:domesticPurchase:trace">
...
$(function(){
	dg=$('#dg').datagrid({
		method: "get",
		url:'${ctx}/domesticTrade/domesticPurchase/json',
		fit : true,
		fitColumns : true,
		border : false,
		striped:true,
		...
		columns:[[
			...
			{field:'supplier',title:'供应商名称',sortable:true,width:20,
		    ...
		    {field:'paymentMethod',title:'支付方式',sortable:true,width:25},
		    {field:'remark',title:'备注',sortable:true,width:25},
...
function add() {
	d=$("#dlg").dialog({
		fit:true,
	    title: '添加采购合同',
	    href:'${ctx}/domesticTrade/domesticPurchase/create',
	    modal:true,
	    closable:false,
	    style:{borderWidth:0},
	    buttons:[{
...
function upd() {
	...
	d=$("#dlg").dialog({
		fit:true,
	    title: '修改采购合同',
	    href:'${ctx}/domesticTrade/domesticPurchase/update/' + row.id,
	    modal:true,
	    closable:false,
	    style:{borderWidth:0},
	    buttons:[{
...
function del(){
	...
	parent.$.messager.confirm('提示', '删除后无法恢复您确定要删除？', function(data){
		if (data){
			$.ajax({
				type:'get',
				url:"${ctx}/domesticTrade/domesticPurchase/delete/" + row.id,
				success: function(data){
					successTip(data,dg);
				}
			});
...
function detail(){
	...
	d=$("#dlg").dialog({
		fit:true,
	    title: '查看明细',
	    href:'${ctx}/domesticTrade/domesticPurchase/detail/' + row.id,
	    modal:true,
	    closable:false,
	    style:{borderWidth:0},
	    buttons:[{
...
function apply(id){
	...
	applyDlg=$("#applyDlg").dialog({
		...
		width: 680,
	    height: 300,
	    href:"${ctx}/domesticTrade/domesticPurchase/apply/" + id,
	    maximizable:false,
	    modal:true
	});
}
...
function callBack(){
	...
	parent.$.messager.confirm('提示', '您确定要撤回申请？', function(data){
		if (data){
			...
			$.ajax({
				type:'get',
				url:"${ctx}/domesticTrade/domesticPurchase/callBack/" + row.id + "/" + row.processInstanceId,
				success: function(data){
			    	successTip(data,dg);
					if(data=='success'){
