...
$(function(){
	dg=$('#dg').datagrid({
		...
		columns:[[
			...
			{field:'customerId',title:'供应商',sortable:true,width:20,
			...
			{field:'paymentMethod',title:'支付方式',sortable:true,width:20},
...
function add() {
	d=$("#dlg").dialog({
		fit:true,
	    title: '添加采购合同',
	    href:'${ctx}/selfRun/purchase/create',
	    modal:true,
	    closable:false,
	    style:{borderWidth:0},
	    buttons:[{
...
function upd() {
	...
	d=$("#dlg").dialog({
		fit:true,
	    title: '修改采购合同',
	    href:'${ctx}/selfRun/purchase/update/' + row.id,
	    modal:true,
	    closable:false,
	    style:{borderWidth:0},
	    buttons:[{
...
function del(){
	...
	parent.$.messager.confirm('提示', '删除后无法恢复您确定要删除？', function(data){
		if (data){
			$.ajax({
				type:'get',
				url:"${ctx}/selfRun/purchase/delete/" + row.id,
				success: function(data){
					successTip(data,dg);
				}
			});
...
function detail(){
	...
	d=$("#dlg").dialog({
		fit:true,
	    title: '查看明细',
	    href:'${ctx}/selfRun/purchase/detail/' + row.id,
	    modal:true,
	    closable:false,
	    style:{borderWidth:0},
	    buttons:[{
...
function apply(id){
	...
	applyDlg=$("#applyDlg").dialog({
		...
		width: 680,
	    height: 300,
	    href:"${ctx}/selfRun/purchase/apply/" + id,
	    maximizable:false,
	    modal:true
	});
}
...
function callBack(){
	...
	parent.$.messager.confirm('提示', '您确定要撤回申请？', function(data){
		if (data){
			...
			$.ajax({
				type:'get',
				url:"${ctx}/selfRun/purchase/callBack/" + row.id + "/" + row.processInstanceId,
				success: function(data){
			    	successTip(data,dg);
					if(data=='success'){
