...
$(function(){   
	dg=$('#dg').datagrid({  
		method: "get",
	    url:'${ctx}/logistics/logisticsCheck/json', 
	    fit : true,
		fitColumns : true,
		border : false,
		striped:true,	
	    ...
	    columns:[[    
			...
			{field:'companyName',title:'公司名称',sortable:true,width:25}, 
			{field:'companyAddress',title:'公司地址',sortable:true,width:30},
			{field:'contactPerson',title:'联系人',sortable:true,width:10},
			{field:'phoneContact',title:'联系电话',sortable:true,width:15},
			{field:'applyDepart',title:'申请部门',sortable:true,width:15},
...
function add() {
	d=$("#dlg").dialog({   
	    title: '添加审核',    
	    href:'${ctx}/logistics/logisticsCheck/create',
	    modal:true,
	    fit:true,
	    style:{borderWidth:0},
	    closable:false,
...
function del(){
	...
	parent.$.messager.confirm('提示', '删除后无法恢复您确定要删除？', function(data){
		if (data){
			$.ajax({
				type:'get',
				url:"${ctx}/logistics/logisticsCheck/delete/"+row.id,
...
function upd(){
	...
	d=$("#dlg").dialog({   
	    ...
	    closable:false,
	    style:{borderWidth:0},
	    href:'${ctx}/logistics/logisticsCheck/update/'+row.id,
	    modal:true,
	    buttons:[{
			text:'提交申请',
			handler:function(){
...
function detail(){
	...
	d=$("#dlg").dialog({   
	    ...
	    style:{borderWidth:0},
	    closable:false,
	    href:'${ctx}/logistics/logisticsCheck/detail/'+row.id,
	    modal:true,
	    buttons:[
	             {
			text:'关闭',
...
function apply(id){
	...
	applyDlg=$("#applyDlg").dialog({
		...
		width: 680,
	    height: 300,
	    href:"${ctx}/logistics/logisticsCheck/apply/" + id,
	    maximizable:false,
	    modal:true
	});
}
...
function callBack(){
	...
	parent.$.messager.confirm('提示', '您确定要撤回申请？', function(data){
		if (data){
			...
			$.ajax({
				type:'get',
				url:"${ctx}/logistics/logisticsCheck/callBack/"+row.id+"/"+row.processInstanceId,
				success: function(data){
			    	successTip(data,dg);
					if(data=='success'){
