...
<div id="tb" style="padding:5px;height:auto">
	<form id="searchFrom" action="">
	    ...
	    <input type="text" name="filter_LIKES_logisticsUnitName" class="easyui-validatebox" data-options="prompt: '物流单位'"/>
...
$(function(){
	dg=$('#dg').datagrid({
		method: "get",
		url:'${ctx}/logistics/freight/json',
		fit : true,
		fitColumns : true,
		border : false,
		striped:true,
		...
		columns:[[
			...
			{field:'logisticsUnitName',title:'物流公司',sortable:true,width:20},
		    {field:'logisticsUnitContacts',title:'联系人',sortable:true,width:20},
			{field:'logisticsUnitContactsPhone',title:'电话',sortable:true,width:20},
...
function add() {
	d=$("#dlg").dialog({
		fit:true,
	    title: '添加货代',
	    href:'${ctx}/logistics/freight/create',
	    modal:true,
	    closable:false,
	    style:{borderWidth:0},
	    buttons:[{
...
function upd() {
	...
	d=$("#dlg").dialog({
		fit:true,
	    title: '修改货代',
	    href:'${ctx}/logistics/freight/update/' + row.id,
	    modal:true,
	    closable:false,
	    style:{borderWidth:0},
	    buttons:[{
...
function del(){
	...
	parent.$.messager.confirm('提示', '删除后无法恢复您确定要删除？', function(data){
		if (data){
			$.ajax({
				type:'get',
				url:"${ctx}/logistics/freight/delete/" + row.id,
...
function detail(){
	...
	d=$("#detailDlg").dialog({
		...
		height : 500,
	    title: '查看明细',
	    href:'${ctx}/logistics/freight/detail/' + row.id,
	    modal:true,
	    closable:false,
	    buttons:[{
			text:'打印',
...
function apply(id){
	...
	applyDlg=$("#applyDlg").dialog({
		...
		width: 680,
	    height: 300,
	    href:"${ctx}/logistics/freight/apply/" + id,
	    maximizable:false,
	    modal:true
	});
}
...
function callBack(){
	...
	parent.$.messager.confirm('提示', '您确定要撤回申请？', function(data){
		if (data){
			...
			$.ajax({
				type:'get',
				url:"${ctx}/logistics/freight/callBack/" + row.id + "/" + row.processInstanceId,
				success: function(data){
			    	successTip(data,dg);
					if(data=='success'){
