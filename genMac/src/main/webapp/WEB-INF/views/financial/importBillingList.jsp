...
<div id="tb" style="padding:5px;height:auto">
	<form id="searchFrom" action="">
	    <input type="text" name="filter_LIKES_importBillingNo" class="easyui-validatebox" data-options="prompt: '结算联系单号'"/>
	    <input type="text" name="filter_LIKES_unitName" class="easyui-validatebox" data-options="prompt: '购货单位名称'"/>
...
$(function(){
	dg=$('#dg').datagrid({
		method: "get",
		url:'${ctx}/financial/importBilling/json',
		fit : true,
		fitColumns : true,
		border : false,
		striped:true,
		...
		columns:[[
			...
			{field:'unitName',title:'购货单位名称',sortable:true,width:20},
			{field:'address',title:'地址',sortable:true,width:20},
		    {field:'phone',title:'电话',sortable:true,width:20},
		    {field:'taxNo',title:'税务登记号',sortable:true,width:20},
...
function add() {
	d=$("#dlg").dialog({
		fit:true,
	    title: '添加进口结算联系单',
	    href:'${ctx}/financial/importBilling/create',
	    modal:true,
	    closable:false,
	    style:{borderWidth:0},
	    buttons:[{
...
function upd() {
	...
	d=$("#dlg").dialog({
		fit:true,
	    title: '修改进口结算联系单',
	    href:'${ctx}/financial/importBilling/update/' + row.id,
	    modal:true,
	    closable:false,
	    style:{borderWidth:0},
	    buttons:[{
...
function del(){
	...
	parent.$.messager.confirm('提示', '删除后无法恢复您确定要删除？', function(data){
		if (data){
			$.ajax({
				type:'get',
				url:"${ctx}/financial/importBilling/delete/" + row.id,
...
function detail(){
	...
	d=$("#dlg").dialog({
		fit:true,
	    title: '查看明细',
	    href:'${ctx}/financial/importBilling/detail/' + row.id,
	    modal:true,
	    style:{borderWidth:0},
	    closable:false,
	    buttons:[{
...
function apply(id){
	...
	applyDlg=$("#applyDlg").dialog({
		...
		width: 680,
	    height: 300,
	    href:"${ctx}/financial/importBilling/apply/" + id,
	    maximizable:false,
	    modal:true
	});
}
...
function callBack(){
	...
	parent.$.messager.confirm('提示', '您确定要撤回申请？', function(data){
		if (data){
			...
			$.ajax({
				type:'get',
				url:"${ctx}/financial/importBilling/callBack/" + row.id + "/" + row.processInstanceId,
				success: function(data){
			    	successTip(data,dg);
					if(data=='success'){
