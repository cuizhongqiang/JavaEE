...
(function ($) {
    ...
    function setTheme(jq, theme, callback, thisArg) {
        ...
        /*add by czq*/
        var orgchartLink = jq("link[href$='jquery.orgchart.css'],link[href*='jquery.orgchart.css']"),
        	orgchartHref = orgchartLink.attr("href"), orgchartArray = orgchartHref ? orgchartHref.split("/") : [];
        if (arguments.length > 1) {
            array[array.length - 2] = theme;
            orgchartArray[orgchartArray.length - 2] = theme;
        } else {
            jq.array.insert(array, 0, theme);
            jq.array.insert(orgchartArray, 0, theme);
        }
        link.attr("href", array.join("/"));
        orgchartLink.attr("href", orgchartArray.join("/"));
